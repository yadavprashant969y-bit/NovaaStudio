...
export class CanvasCompositor {
    ...
    for (const track of videoTracks) {
      for (const clip of track.clips) {
        if (currentTime >= clip.startTime && currentTime <= (clip.startTime + clip.duration)) {
          ...
          if (clip.type === 'image') {
            await this.renderImageClip(clip, false);
          } else {
            await this.renderVideoClip(clip, clipTime, false);
          }
        }
      }
    }
    ...
    for (const track of pipTracks) {
      for (const clip of track.clips) {
        if (currentTime >= clip.startTime && currentTime <= (clip.startTime + clip.duration)) {
          const clipTime = (currentTime - clip.startTime) * (clip.speed || 1) + (clip.trimStart || 0);
          if (clip.type === 'image') {
            await this.renderImageClip(clip, true);
          } else {
  ...
  async getOrCreateImageElement(url) {
    if (!this.imageElements) this.imageElements = new Map();
  ...
  async renderImageClip(clip, isPiP = false) {
    const ctx = this.ctx;
    const img = await this.getOrCreateImageElement(clip.mediaUrl || clip.url);

    ctx.save();
    
    // Support filters on images
    ...
    if (filters.preset === 'cinematic') filterString += ' contrast(115%) saturate(120%) brightness(95%)';
    else if (filters.preset === 'cyberpunk') filterString += ' hue-rotate(290deg) saturate(180%) contrast(130%)';
    else if (filters.preset === 'vintage') filterString += ' sepia(50%) contrast(90%) brightness(105%)';
    else if (filters.preset === 'bw') filterString += ' grayscale(100%) contrast(120%)';
    ctx.filter = filterString;

    let destX = 0, destY = 0, destW = this.width, destH = this.height;

    if (isPiP) {
      destW = this.width * (clip.scale || 0.35);
      destH = (destW / (img.width || 16)) * (img.height || 9);
      ...
      const iRatio = (img.width || 16) / (img.height || 9);
      const cRatio = this.width / this.height;
      if (iRatio > cRatio) {
        destH = this.height;
        destW = this.height * iRatio;
        destX = (this.width - destW) / 2;
      } else {
        destW = this.width;
        destH = this.width / iRatio;
