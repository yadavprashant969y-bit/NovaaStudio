...
export class TimelineManager {
  ...
  initEventListeners() {
    ...
    if (this.rulerCanvas) {
      ...
      this.rulerCanvas.addEventListener('mousedown', startScrub);
      this.rulerCanvas.addEventListener('touchstart', startScrub, { passive: true });
  ...
  makeClipDraggable(clipEl, clip) {
    ...
    const startDrag = (e) => {
      ...
      const onMove = (moveEv) => {
        ...
        const clientX = moveEv.type.includes('touch') ? moveEv.touches[0].clientX : moveEv.clientX;
  ...
  makeClipTrimmable(leftHandle, rightHandle, clip) {
    ...
    leftHandle.addEventListener('mousedown', startLeftTrim);
    leftHandle.addEventListener('touchstart', startLeftTrim, { passive: true });

    // Right trim
    const startRightTrim = (e) => {
      e.stopPropagation();
      let startX = e.type.includes('touch') ? e.touches[0].clientX : e.clientX;
      let initialDuration = clip.duration;
    ...
    rightHandle.addEventListener('mousedown', startRightTrim);
    rightHandle.addEventListener('touchstart', startRightTrim, { passive: true });
  }
}
