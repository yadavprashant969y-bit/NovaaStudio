...
class NovaCutApp {
  ...
  initUI() {
    ...
    document.getElementById('mobileNavMedia')?.addEventListener('click', (e) => {
      if (leftSidebar.classList.contains('show-mobile')) {
        leftSidebar.classList.remove('show-mobile');
        e.currentTarget.classList.remove('active');
      } else {
        leftSidebar.classList.add('show-mobile');
        rightInspector.classList.remove('show-mobile');
        e.currentTarget.classList.add('active');
        document.getElementById('mobileNavInspector')?.classList.remove('active');
      }
    });

    document.getElementById('mobileNavInspector')?.addEventListener('click', (e) => {
      if (rightInspector.classList.contains('show-mobile')) {
        rightInspector.classList.remove('show-mobile');
        e.currentTarget.classList.remove('active');
      } else {
        rightInspector.classList.add('show-mobile');
        leftSidebar.classList.remove('show-mobile');
        e.currentTarget.classList.add('active');
        document.getElementById('mobileNavMedia')?.classList.remove('active');
      }
    });

    document.getElementById('mobileNavExport')?.addEventListener('click', () => {
      exportModal.classList.add('open');
    });
  ...
  handleUserUploadedFiles(files) {
    files.forEach(file => {
      ...
      const isAudio = file.type.startsWith('audio/') || file.name.match(/\.(mp3|wav|m4a|ogg|aac)$/i);
      ...
      if (isVideo) {
        ...
        video.onloadedmetadata = () => {
          ...
          this.showToast(`Imported video ${file.name}`);
        };
        video.load();
      } else if (isAudio) {
        ...
        const finishLoad = () => {
          ...
          const data = {
            title: file.name,
            url,
            duration: Math.max(1, Math.round(audio.duration) || 30),
            genre: 'USER AUDIO'
          };
        ...
        img.onload = () => {
          const data = {
            title: file.name,
            url,
            duration: 5, // Default 5 seconds for photos
            thumbnail: url
          };
          this.addMediaCardToBin(data, 'video'); // Add to media bin
          this.timeline.addClip('track-video-1', { ...data, type: 'image' });
          this.showToast(`Imported photo ${file.name}`);
        };
      } else {
